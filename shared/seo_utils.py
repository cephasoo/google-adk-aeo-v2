...
def audit_fact_integrity(draft_content, context, model, gen_func, output_target="MODERATOR_VIEW"):
    ...
    OUTPUT FORMAT:
    - If Target is SLACK/MODERATOR: Return a concise text block with Markdown.
    - If Target is CMS/GHOST: Return a clean HTML <div> block with class="aeo-audit-box".
    """
    try:
        audit_resp = gen_func(model, audit_prompt)
        
        # Post-process for Ghost HTML if needed
        if output_target == "CMS_DRAFT" and "<div" not in audit_resp:
            # Wrap in HTML if the LLM returned raw text
            return f"""
            <div class="aeo-audit-shield" style="margin-top:20px; padding:15px; border:1px solid #e1e1e1; border-radius:8px; background:#f9f9f9; font-family: sans-serif; font-size: 0.9em;">
                <h4 style="margin-top:0; color:#333;">🛡️ Attribution Integrity Audit</h4>
                <div style="white-space: pre-wrap;">{html.escape(audit_resp)}</div>
            </div>
            """
        return audit_resp
    except Exception as e:
