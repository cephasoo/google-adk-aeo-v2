    ...
    if name == "google_web_search":
        ...
        try:
            ...
            if is_pdf:
                ...
                model = get_flash_model() # Uses gemini-2.0-flash (or whichever is configured)
        ...
        **CRITICAL**: 
        ...
        try:
            ...
            try:
                # Validate the URL to prevent 400 errors from rendering broken images on the frontend
                resilient_request("GET", image_url, timeout=10)
                
                if output_format == "html":
                    result = f'<figure class="kg-card kg-image-card kg-width-wide"><img src="{image_url}" class="kg-image" alt="{alt_text}" loading="lazy"><figcaption>{alt_text}</figcaption></figure>'
                else:
                    # RAW URL for Slack unfurling 🛡️📊🛡️
                    result = f"\n\n{image_url}\n\n"
            except Exception as validate_error:
                print(f"⚠️ Mermaid Diagram Validation Failed (Likely invalid syntax): {validate_error}")
                # Syntax error generated by LLM -> Fallback to raw code
                if output_format == "html":
                    result = f'<pre><code class="language-mermaid">\n{mermaid_code}\n</code></pre>'
                else:
                    result = f"```mermaid\n{mermaid_code}\n```"
        except Exception as e:
